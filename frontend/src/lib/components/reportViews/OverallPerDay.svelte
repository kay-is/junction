<script lang="ts">
  ...
  const aggregateRecords = (records: typeof report.currentRecordsArray) => {
  ...
  $effect(() => {
    if (!chartCanvas || !report) return

    const currentRecords = aggregateRecords(report.currentRecordsArray)
    const referenceRecords = aggregateRecords(report.referenceRecordsArray)
    ...
    const referenceEnd = Math.max(...referenceRecords.map((r) => r.timestamp))
...
<Card class="col-span-3 min-h-40 w-full max-w-none overflow-clip">
  <Heading tag="h4">Overall</Heading>
